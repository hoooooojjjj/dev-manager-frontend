...
- **G1. 인입 명시화**: Notion PRD·회의록과 \*\*GitHub 레포 + 중점 파일 목록(`focusFiles[]`)\*\*을 입력으로 받아 자동 수집·정규화.
...
- **시나리오**
  1. 사용자가 `notionUrl`, `repo`, `focusFiles[]`, `outputNotionUrl`을 제출.
...
- 입력: `notionUrl`, `repo`, `focusFiles[]`, `outputNotionUrl`, `title(optional)`
- OAuth 상태 확인(노션/깃허브)

1. **수집/분석 파이프라인**(서버) →

- Notion MCP: 페이지/하위 블록 Markdown Export
- GitHub MCP: 레포 트리, 최근 N PR/커밋/이슈, **focusFiles AST/metrics**
...
- \*\AC3.2 \\\*\*\*`focusFiles`\*\***에 대해 AST/Complexity/변화이력/주요 시그니처 추출**
...
- `user(id, email, display_name, createdAt)`
- `project(id, userId, title, notionUrl, repo, outputNotionUrl, status)`
...
- `spec_draft(id, project_id, version, json, md, quality_score, createdAt)`
- `ai_prompt(id, project_id, draft_version, kind[codegen|test|review], content_md, createdAt)`
- `feedback(id, draft_id, section_key, role[user|ai], message, createdAt)`
- `iteration(id, project_id, from_version, to_version, diff_json, createdAt)`
- `publish_log(id, project_id, draft_version, notion_page_id, url, createdAt)`
- `oauth_token(id, userId, provider, scope, enc_token, refreshed_at)`
...
```json
{
  "notionUrl": "string",
  "repo": "owner/name",
  "focusFiles": ["src/foo.ts", "app/page.tsx"],
...
- **목적**: AI와의 협업을 통한 섹션별 수정, 실시간 Diff 확인
- **구현**: 대화형 UX 디자인 (2+1 영역 레이아웃)
  - 좌측: 섹션 선택 패널 (완료/미완료 상태 배지)
  - 우측: 수정 지시문 입력 및 리뷰 제출 폼
  - 하단: 동적 콘텐츠 (섹션 내용 → Diff 뷰 → 승인/되돌리기)
- **워크플로우**: 섹션 선택 → 지시문 입력 → AI 처리 → Before/After 확인 → 승인/되돌리기
- **기능**: 실시간 Diff, 엄격한 인용 검증, 변경 이력 관리
