import { AlertTriangle, Code, Lightbulb, Search, Target } from 'lucide-react';
...
export const spec = {
  tldr: `세그먼트 기반 트랙 큐레이션 시스템 구현으로 신규 사용자 전환율 개선. 주요 개선사항: (1) 사용자 세그먼트별 맞춤형 트랙 분류, (2) 키워드 기반 트랙 매핑 시스템, (3) 세그먼트 탭 UI 구현. 예상 개발 기간: 2주, 전환율 목표: 15% 향상.`,
  current_behavior: `내일배움캠프 허브 페이지에 유입된 신규 사용자들은 마감 임박 프로그램 외에 어떤 트랙을 선택해야 할지, 무엇을 기준으로 판단해야 할지에 대한 명확한 가이드를 받지 못하고 있습니다. 
...
이로 인해, 트랙에 관심은 있으나 적절한 전환 동기 없이 많은 사용자들이 트랙 상세 페이지(PV)로 전환되지 않고 이탈하고 있는 실정입니다.`,
  root_cause: `현재 구현된 컴포넌트(\`src/app/pages/Herb/v2/components/NBBootcampsWithBoosting/template.tsx\`)는 트랙 카드를 단순히 세 가지 카테고리(부스팅/개발/비개발)로만 분류하여 제공하고 있습니다.
...
3. **사용자 세그먼트 기반 추천 부재**: 현재 코드는 사용자의 직업, 관심사, 경력 목표 등 세그먼트 정보를 활용한 맞춤형 트랙 추천 기능이 없음`,
  solutions: `프로젝트 브리핑의 실험 설계 중 C안(마감 임박 + 세그먼트 기반 트랙 탐색)을 중심으로 키워드 기반 트랙 큐레이션 기능을 구현하여 사용자에게 맞춤형 가이드를 제공하는 방안을 제안합니다.
...
- **개발자 경험**: TypeScript 타입 정의로 코드 안정성 확보 및 향후 확장성 고려`,
  learning_points: `## 1. 세그먼트 기반 개인화 추천 구현 역량
